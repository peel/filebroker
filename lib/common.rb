...
class System
  ...
  def compress(transfer_id, file, level = 0)
	  begin
      root = Dir.pwd
      Dir.chdir("process/#{transfer_id}")

      self.exec("zip -#{level} 'files_#{transfer_id}.zip' '#{file}'")
  ...
  def decompress(transfer_id, file)
      ...
      if file =~ /\.tar\.bz2$/
        self.exec("tar jxf '#{file}'")
      elsif file =~ /\.tar\.gz$/
        self.exec("tar zxf '#{file}'")
      elsif file =~ /\.tar$/
        self.exec("tar xf '#{file}'")
      elsif file =~ /\.bz2$/
        self.exec("bzip2 -d '#{file}'")
      elsif file =~ /\.gz$/
        self.exec("gzip -d '#{file}'")
      elsif file =~ /\.zip$/
        self.exec("unzip -o '#{file}'")
      else
  ...
  def archive(transfer_id, file)
    begin
      root = Dir.pwd
      Dir.chdir("process/#{transfer_id}")
      self.exec("zip -3 '../../archive/#{transfer_id}' '#{file}'")
    rescue
  ...
  def initialize
	...
	def decrypt(file)
      ...
      if file =~ /^(.+)\.([gpg|pgp|asc]{3})$/
        out = `gpg --no-tty --homedir #{@rootdir}/etc/gnupg --batch -o \"#{$1}\" --decrypt \"#{$1}.#{$2}\" 2>&1`
        raise StandardError, 'failed to decrypt file' if out.split("\n").last =~ /gpg: decrypt_message failed: eof/
        File.unlink(file)
      else
        out = `"gpg --no-tty --homedir #{@rootdir}/etc/gnupg --batch -o \"#{file}.out\" --decrypt \"#{file}\"`
        raise StandardError, 'failed to decrypt file' if out.split("\n").last =~ /gpg: decrypt_message failed: eof/
    ...
    def list(path)
      begin
        ...
        lines[0..(lines.length - 2)].each { |line|
          ...
          words = line.split(' ')
          next if words[(words.length - 7)].split('').include?('D')
